...
import { LogLevel, LoggerConfig, globalLogger, getLogger } from './logger';
import { JoinSource } from './queryObject';
...
export class DataGateway
{
	...
	/**
	 * Resolves a JoinSource to a table name.
	 * @param source The join source to resolve.
	 * @returns The resolved table name, or null if not found.
	 */
	joinSourceToTableName(source: JoinSource): string | null
	{
		if ('table' in source)
		{
			return source.table;
		...
		{
			const repo = this.getRepository(source.repository);
			return repo ? repo.getTable() : null;
		}
